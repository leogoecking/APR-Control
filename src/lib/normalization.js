...
export function normalizeDateValue(value, xlsxRef = globalThis.XLSX) {
  if (value === null || value === undefined || value === '') return '';
  if (typeof value === 'number' && xlsxRef?.SSF) {
    try {
      const parsed = xlsxRef.SSF.parse_date_code(value);
      if (parsed && parsed.y && parsed.m && parsed.d) {
        const isoFromNumber = `${String(parsed.y).padStart(4, '0')}-${String(parsed.m).padStart(2, '0')}-${String(parsed.d).padStart(2, '0')}`;
  ...
  if (trimmedDatePart) {
    const isoDateOnly = normalizeDateValue(trimmedDatePart[1], xlsxRef);
...
export function canonicalEmployeeMatch(value, candidates = DEFAULT_EMPLOYEES) {
  ...
  candidates.forEach(candidate => {
    ...
    candidateTokens.forEach(token => {
      if (rawTokenSet.has(token)) common++;
    });
